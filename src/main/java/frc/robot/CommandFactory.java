...
public class CommandFactory {
    ...
    public Command aimShooter(BooleanSupplier aimAtHub) {
        ...
        Supplier<Pose2d> target = () -> getShooterTarget(drive.getPose(), redAlliance.getAsBoolean(), aimAtHub.getAsBoolean());

        return shooter.followWithShooter(
                () -> getAngle(drive.getPose(), target.get()),
                () -> getHoodAngle(drive.getPose(), target.get(), getAngle(drive.getPose(), target.get()), aimAtHub.getAsBoolean())
        );
    }
    ...
    public double getHoodAngle(Pose2d robot, Pose2d target, double turretAngle, boolean aimHigh) {
        ...
        double distance = Math.sqrt(turretOffset * turretOffset + robotD * robotD - 2 * turretOffset * robotD * Math.cos(turretToTargetAngle));
        double height = aimHigh ? Constants.Shooter.HUB_HEIGHT : Constants.Shooter.PASS_HEIGHT;
        return getLaunchAngle(9.81, distance, height, shooter.getShootingOutputVelocity(), Constants.Shooter.LAUNCH_HEIGHT, true);
    }
    ...
    public double getLaunchAngle(double gravity, double distance, double targetHeight, double launchVelocity, double launchHeight, boolean highShot) {
